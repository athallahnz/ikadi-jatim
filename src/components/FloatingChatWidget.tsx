...
const FloatingChatWidget: React.FC<FloatingChatWidgetProps> = ({
  ...
  return (
    <aside className="fixed bottom-5 right-5 z-[100] font-sans flex flex-col items-end">
      {isChatOpen ? (
        <div className="bg-white dark:bg-emerald-950 w-[calc(100vw-40px)] sm:w-[400px] md:w-[440px] h-[min(650px,80vh)] shadow-[0_20px_50px_rgba(6,78,59,0.2)] rounded-3xl border border-emerald-100 dark:border-emerald-800/50 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-300 origin-bottom-right">
          <header className="bg-gradient-to-br from-emerald-600 to-emerald-800 p-4 md:p-6 text-white relative shadow-lg shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 md:h-12 md:w-12 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/30">
                  <Bot className="h-5 w-5 md:h-6 md:w-6" />
                </div>
                <div>
                  <h3 className="font-semibold text-base md:text-lg leading-tight tracking-tight">
                    Konsultasi Syariah AI
                  </h3>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="h-1.5 w-1.5 bg-emerald-300 rounded-full animate-pulse"></span>
                    <span className="text-[10px] md:text-[11px] font-medium text-emerald-100 uppercase tracking-wider">
                      AI Assistance
                    </span>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={clearChat}
                  className="h-9 w-9 md:h-10 md:w-10 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors group"
                ...
                <button
                  ...
                  className="h-9 w-9 md:h-10 md:w-10 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors"
                >
          ...
          <div
            ref={scrollRef}
            className="flex-1 p-4 md:p-6 overflow-y-auto bg-emerald-50/30 dark:bg-emerald-900/10 space-y-6 custom-scrollbar scroll-smooth"
          >
            {messages.map((msg, index) => (
              <div key={msg.id} className="space-y-4">
                ...
                >
                  ...
                  <div
                    className={`p-3 md:p-4 shadow-sm text-[13px] md:text-[13.5px] leading-relaxed transition-all duration-300 max-w-[90%] ${
                      msg.role === "user"
                  ...
                  >
                    {msg.role === "assistant" ? (
                      <div className="prose prose-sm dark:prose-invert prose-emerald max-w-none prose-p:leading-relaxed prose-p:mb-2 last:prose-p:mb-0 prose-ul:list-disc prose-ul:ml-4 prose-ol:ml-4 prose-strong:text-emerald-700 dark:prose-strong:text-emerald-400">
                        <ReactMarkdown
                          components={{
                            a: ({ href, children }) => (
                              <button
                  ...
                  shouldShowConsultButton(msg.content) && (
                    <div className="flex flex-col items-start gap-3 animate-in fade-in slide-in-from-left-4 duration-700 ml-1">
                      ...
                      <button
                        ...
                        className="relative px-5 py-2.5 rounded-2xl text-[11px] md:text-[12px] font-extrabold text-white bg-gradient-to-r from-emerald-700 to-emerald-600 hover:from-emerald-600 hover:to-emerald-500 shadow-lg shadow-emerald-600/20 active:scale-95 transition-all"
                      >
          ...
          <footer className="p-4 md:p-5 bg-white dark:bg-emerald-950 border-t border-emerald-100 dark:border-emerald-900 shrink-0">
            <div className="relative group">
              <textarea
                ...
                disabled={isSending}
                className="w-full bg-emerald-50/50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800 rounded-2xl p-3 md:p-4 pr-12 md:pr-14 text-[13px] md:text-[13.5px] transition-all resize-none min-h-[80px] md:min-h-[90px] focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none disabled:opacity-50 text-emerald-950 dark:text-emerald-50"
              />
              <Button
                onClick={() => handleSubmitChat()}
                disabled={isSending || !chatMessage.trim()}
                size="icon"
                className="absolute bottom-3 right-3 h-9 w-9 md:h-10 md:w-10 rounded-xl bg-emerald-600 hover:bg-emerald-700 shadow-lg transition-all active:scale-95"
              >
                {isSending ? (
        ...
        <div className="relative">
          ...
          >
            ...
            <p className="font-medium opacity-90">
              ...
              <br />
              kepada Robot Syariah AI kami.
            </p>
            <div className="absolute top-full right-6 md:right-8 -translate-y-1/2 rotate-45 h-2.5 w-2.5 bg-emerald-900 border-r border-b border-emerald-800/50"></div>
          </div>

          <Button
            onClick={() => setIsChatOpen(true)}
            size="lg"
            className="relative rounded-full h-14 w-14 md:h-20 md:w-20 shadow-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 hover:from-emerald-600 hover:to-emerald-800 transition-all duration-500 hover:scale-110 active:scale-95 flex items-center justify-center p-0 border-4 border-white dark:border-emerald-900 group"
          >
            <Bot
              className="h-7 w-7 md:h-11 md:w-11 transition-all duration-500 group-hover:rotate-12 group-hover:scale-110 text-white drop-shadow-md relative z-10"
              strokeWidth={1.5}
            />
          </Button>
        </div>
